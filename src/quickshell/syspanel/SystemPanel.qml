...
Item {
    ...
    property real sysExtBrightness: 0
    property bool isDraggingExtBri: false
    property bool extBriReady: false
    ...
    onVisibleChanged: {
        ...
        if (visible) {
            ...
            extBriPollerTimer.interval = 150;
            extBriPollerTimer.restart();
        } else {
            briPollerTimer.stop();
            briPoller.running = false;
            extBriPollerTimer.stop();
            extBriPoller.running = false;
    ...
    Timer {
        id: extBriPollerTimer
        interval: 10000
        repeat: false
        onTriggered: {
            if (root.visible) {
		extBriPoller.running = true;
		interval = 10000;
	    }
        }
    }
    ...
    Process {
        id: extBriPoller
        command: ["bash", Caching.qsDir + "/../scripts/brightness.sh", "get_ext"]
        running: false
        stdout: StdioCollector {
            onStreamFinished: {
                let out = this.text.trim();
                if (out !== "") {
                    root.extBriReady = true;
                    if (!root.isDraggingExtBri) {
                        root.sysExtBrightness = parseInt(out) || 0;
                    }
                }
                if (root.visible) extBriPollerTimer.start();
            }
        }
    }
                    ...
                    ColumnLayout {
                        ...
                        RowLayout {
                            ...
                            IconButton {
                                ...
                                textColor: isHoveredOrHighlighted ? ThemeBackend.text : root.briColor
                                onClicked: {
                                    extBriCmdThrottle.stop();
                                    extBriCmdThrottle.targetPct = -1;
                                    let target = root.sysExtBrightness > 0 ? 0 : 100;
                                    root.sysExtBrightness = target;
                                    Quickshell.execDetached(["bash", Caching.qsDir + "/../scripts/brightness.sh", "set_ext", target.toString()]);
                                }
                            }

                            Timer {
                                id: extBriCmdThrottle
                                interval: 400
                                property int targetPct: -1
                                onTriggered: {
                                    if (targetPct >= 0) {
                                        Quickshell.execDetached(["bash", Caching.qsDir + "/../scripts/brightness.sh", "set_ext", targetPct.toString()]);
                                        targetPct = -1;
                                    }
                                }
                            }
                            ...
                            Draggable {
                                ...
                                onDragStarted: {
                                    root.isDraggingExtBri = true;
                                }
                                onDragFinished: {
                                    root.isDraggingExtBri = false;
                                    if (extBriCmdThrottle.targetPct >= 0) {
                                        extBriCmdThrottle.stop();
                                        Quickshell.execDetached(["bash", Caching.qsDir + "/../scripts/brightness.sh", "set_ext", extBriCmdThrottle.targetPct.toString()]);
                                        extBriCmdThrottle.targetPct = -1;
                                    }
                                }
                                onMoved: (val) => {
                                    let pct = Math.max(0, Math.min(100, Math.round(val)));
                                    root.sysExtBrightness = pct;
                                    extBriSlider.value = pct;
                                    extBriCmdThrottle.targetPct = pct;
                                    if (!extBriSlider.isDragging) extBriCmdThrottle.restart();
                                }
                            }
                        }			
                    }
